...
function MapRoute({
  ...
  const layerIdBg = `route-layer-bg-${id}`;
  ...
  useEffect(() => {
    ...
    const handleStyleData = () => {
      // Re-add on style change if missing
      if (!map.getSource(sourceId)) {
        addRoute();
      }
    };
    ...
    return () => {
      ...
      try {
        ...
        if (map.getLayer(layerIdBg)) map.removeLayer(layerIdBg);
        if (map.getSource(sourceId)) map.removeSource(sourceId);
      } catch {
        // ignore
      }
    };
  }, [isLoaded, map, sourceId, layerId, layerIdBg, coordinates, color, width, opacity, dashArray]);
